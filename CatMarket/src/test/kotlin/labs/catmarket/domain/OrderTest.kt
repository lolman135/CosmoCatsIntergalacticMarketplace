...
class OrderTest {
    ...
    private fun item(price: Int, qty: Int) =
        Order.OrdersItem(UUID.randomUUID(), qty, price)
    ...
    fun shouldMarkAsPaidFromNewAndKeepOtherFields() {
        ...
        assertEquals(order.ordersItems, paid.ordersItems)
    }

    @Test
    fun shouldThrowWhenMarkAsPaidCalledAndStatusIsNotNew() {
        val alreadyPaid = Order(id = null, status = Status.PAID, ordersItems = listOf(item(10, 1)))
        assertThrows(IllegalStateException::class.java) { alreadyPaid.markAsPaid() }

        val canceled = Order(id = null, status = Status.CANCELED, ordersItems = listOf(item(10, 1)))
        assertThrows(IllegalStateException::class.java) { canceled.markAsPaid() }
    }

    @Test
    fun shouldMarkAsShippedFromPaid() {
        val order = Order(id = null, status = Status.PAID, ordersItems = listOf(item(10, 1)))
    ...
    fun shouldThrowWhenMarkAsShippedCalledAndStatusIsNotPaid() {
        val newOrder = Order(id = null, status = Status.NEW, ordersItems = listOf(item(10, 1)))
        assertThrows(IllegalStateException::class.java) { newOrder.markAsShipped() }

        val canceledOrder = Order(id = null, status = Status.CANCELED, ordersItems = listOf(item(10, 1)))
    ...
    fun shouldMarkAsCanceledFromNewOrPaid() {
        ...
        val paidOrder = Order(id = null, status = Status.PAID, ordersItems = listOf(item(10, 1)))
        assertEquals(Status.CANCELED, paidOrder.markAsCanceled().status)
    }

    @Test
    fun shouldThrowWhenMarkAsCanceledCalledFromShipped() {
        val shipped = Order(id = null, status = Status.SHIPPED, ordersItems = listOf(item(10, 1)))
